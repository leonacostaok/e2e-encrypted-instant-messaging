  ...
  return (
    <ModalChat onDismiss={onDismiss} visible={visible} title={'Forward Message'}>
      <ModalChild>
        ...
        <ModalSearch>
          <FormInputSearch placeholder={'Search'} id={'search-user'} name={'search-user'} value={term} onChange={onChange}/>
        </ModalSearch>
        <ModalRecent>
          <TextRecent>Recent</TextRecent>
          <ListUser>
            <ContactItem type={ChatTypeEnum.USER} checkBox={true} dataUser={{alias: 'user100'}} disable={count >= 5}/>
            <ContactItem type={ChatTypeEnum.USER} checkBox={true} dataUser={{alias: 'user101'}} disable={count >= 5}/>
            <ContactItem type={ChatTypeEnum.USER} checkBox={true} dataUser={{alias: 'user102'}} disable={count >= 5}/>
            <ContactItem type={ChatTypeEnum.USER} checkBox={true} dataUser={{alias: 'user103'}} disable={count >= 5}/>
            <ContactItem type={ChatTypeEnum.USER} checkBox={true} dataUser={{alias: 'user104'}} disable={count >= 5}/>
            <ContactItem type={ChatTypeEnum.GROUP} checkBox={true} dataUser={{alias: 'user105'}} disable={count >= 5}/>
          </ListUser>
          {
            count >=5 && <TextWarning>You can forward up to <span>5</span> people or groups at a time</TextWarning>
          }
        </ModalRecent>
        <ModalFooter>

    ...
    <ContactItemBox onClick={handleClickContactItem}>
      <BoxNameAvatar type={type} />
      {
        checkBox && dataUser &&  <CheckBoxCs aria-disabled={disable}>
          <InputCheckBox hidden type="checkbox" name={`checkbox-option-${dataUser.alias}`} id={`checkbox-option-${dataUser.alias}`} onChange={(event:React.ChangeEvent<HTMLInputElement>) => handleChange(event)}/>

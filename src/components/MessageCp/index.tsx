...
const formatChatDate = (date: Date): any => {
    const momentDate = moment.unix(parseInt(date.toString()));
    const momentCurrentDate = moment(new Date());

    if (momentDate.isSame(momentCurrentDate, 'day')) {
        // Format time only if it's the same day
        return momentDate.format('h:mm A');
    } else {
        // Format date and time if it's a different day
...
const MessageCp = ({
    ...
    return (
        <MessageCpWrapper>
            <MessageContent mine={mine}>
                {
                    !mine && !isForward && <AvatarUser>
                        <img src={IconUserDefault as string} alt={'avatar-user'}/>
                    </AvatarUser>
                }
                {
                    isForward && !wrongMessage && <CheckBoxCs>
                        <InputCheckBox hidden type="checkbox" name={`checkbox-option-${id}`} id={`checkbox-option-${id}`}
                                       onChange={(event: React.ChangeEvent<HTMLInputElement>) => handleCheckForward(event)}/>
                ...
                <Detail>
                    ...
                    <DetailBox ref={detailBoxNode}
                        ...
                        {
                            showOptionMess &&
                            <MenuOptionMessage hideMenu={hideMenu} onOpenPopupDelete={onOpenPopupDelete}
                                               onReply={onReply} onForward={onForward} onCopy={onCopy} onEdit={onEdit}
                                               mine={mine}/>
                        }
                        {
                ...
                </Detail>
                {
                    !wrongMessage ? <TimeBox>
                        {mine && <IconPortal srcIcon={IconDelivered as string} widthIcon={'12px'} heightIcon={'7px'}/>}

                        {data?.data?.createdAt ?
                            <Time>{formatChatDate(data?.data?.createdAt)}</Time> : <Time>Time</Time>}
                    </TimeBox> : <StatusMess mine={mine}>
                        <IconPortal srcIcon={IconWrongMessage as string} widthIcon={'14px'} heightIcon={'14px'}/>
                        {data?.data?.createdAt ?
                            <Time>{formatChatDate(data?.data?.createdAt)}</Time> : <Time>Time</Time>}
                    </StatusMess>
                }
            ...
            {
                popupDelete &&
                <ModalDeleteChat onDismiss={onDismissPopupDelete} visible={popupDelete} onDelete={onDelete}/>
            }
        </MessageCpWrapper>
    );
};
